...
import dev.voras.framework.docker.controller.pojo.Container;
import dev.voras.framework.docker.controller.pojo.HostConfig;
import dev.voras.framework.docker.controller.pojo.Labels;
...
public class RunPoll implements Runnable {
	...
	public RunPoll(IDynamicStatusStoreService dss, Settings settings, CloseableHttpClient httpClient, IFrameworkRuns runs) {
		...
		this.submittedRuns = Counter.build()
				.name("cirillo_docker_controller_submitted_runs")
	...
	private void startPod(IRun run) {
		...
		try {
			...
			container.Labels.vorasEngineController = this.settings.getEngineLabel();
			container.Labels.vorasRun              = runName;
			...
			container.Cmd.add("file:voras.obr");
		...
		try {
			...
			try (CloseableHttpResponse response = httpClient.execute(get)) {
				...
				for(JsonElement element : array) {
					...
					if (container.Labels.vorasEngineController == null || container.Labels.vorasRun == null) {
						continue;
					}
