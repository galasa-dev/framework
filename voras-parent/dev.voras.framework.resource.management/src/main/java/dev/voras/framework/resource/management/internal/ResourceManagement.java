...
import dev.voras.framework.TestRunException;
...
public class ResourceManagement implements IResourceManagement {
	...
	public void run(Properties bootstrapProperties, Properties overrideProperties) throws FrameworkException  {
		...
		this.successfulRunsCounter = Counter.build()
				.name("ejat_resource_management_successfull_runs")
				.help("The number of successfull resource management runs")
				.register();
		...
		try {
			final ServiceReference<?>[] rmpServiceReference = bundleContext.getAllServiceReferences(IResourceManagementProvider.class.getName(), null);
			if ((rmpServiceReference == null) || (rmpServiceReference.length == 0)) {
				...
				for (final ServiceReference<?> rmpReference : rmpServiceReference) {
					final IResourceManagementProvider rmpStoreRegistration = (IResourceManagementProvider)bundleContext.getService(rmpReference);
					try {
						if (rmpStoreRegistration.initialise(framework, this)) {
							logger.info("Found Resource Management Provider "+ rmpStoreRegistration.getClass().getName());
							resourceManagementProviders.add(rmpStoreRegistration);
						} else {
							logger.info("Resource Management Provider "+ rmpStoreRegistration.getClass().getName() + " opted out of this Resource Management run");
						}
					} catch(Exception e) {
						logger.error("Failed initialisation of Resource Management Provider " + rmpStoreRegistration.getClass().getName() + " ignoring",e);
					}
				}
			}
		} catch(Exception e) {
