...
public class RunWaitingRuns implements Runnable {
	...
	protected RunWaitingRuns(IFramework framework, IResourceManagement resourceManagement, IDynamicStatusStoreService dss, RunResourceManagement runResourceManagement, IConfigurationPropertyStoreService cps) throws FrameworkException {
			...
			for(IRun run : runs) {
				...
				if (finished.compareTo(now) <= 0) {
					if (this.dss.putSwap("run." + run.getName() + ".status", "waiting", "queued")) {
						//*** Leave the queue time as is as we want the waiting runs to be actioned before 1st time queued runs
