	...
	private void retrieveConfigMap() {
		...
		try {
			...
			if (newEngineImage == null || newEngineImage.trim().isEmpty()) {
				newEngineImage = "cicsts-docker-local.artifactory.swg-devops.com/ejatv3-boot-embedded";
			}
		...
		try {
			String newNodePreferredAffinity = configMap.getData().get("ejat_node_preferred_affinity");
