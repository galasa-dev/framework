...
function update_release_yaml {
    ...
    if [[ -f ${generated_release_yaml} ]]; then
        cp ${generated_release_yaml} ${BASEDIR}/release.yaml
        success "Updated release.yaml OK"
    else
        warn "Failed to automatically generate release.yaml, please ensure any changed bundles have had their versions updated in the release.yaml file"
    fi
}
